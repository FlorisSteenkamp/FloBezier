...
function hitThreshold(viewbox: number[][]): number {
...
function findBezierAt(
        beziers: number[][][], p: number[], threshold: number): number | undefined {
...
function findControlPointAt(
        beziers: number[][][], p: number[], radius: number): [number, number] | undefined {

    const maxDSquared = radius * radius;
    let hit: [number, number] | undefined = undefined;
    let minDSquared = Infinity;
    beziers.forEach((ps, i) => {
        ps.forEach((c, j) => {
            const dSquared = squaredDistanceBetween(c, p);
            if (dSquared < minDSquared) { minDSquared = dSquared; hit = [i, j]; }
        });
    });
...
function tryStartBezierDrag(
        stateControl: StateControl,
        refSvg: React.RefObject<SVGSVGElement | null>,
        event: React.MouseEvent<SVGSVGElement, MouseEvent>): boolean {
    ...
    const p = getViewboxXY(
        svg$, viewbox, event.nativeEvent.offsetX, event.nativeEvent.offsetY);
...
function dragBezier(
    ...
    if (dragState.bezierIdx === undefined) { return; }
    ...
    const newBeziers = beziers.map((ps, i) => {
        ...
        return ps.map(([x, y], j) =>
            (cpIdx === undefined || j === cpIdx) ? [x + dx, y + dy] : [x, y]);
    });
    ...
    if (state.appState.pageState.liveUpdate) { stateControl.render(); }
}


/** Ends an in-progress drag, persisting the final position. Returns `true` if dragging. */
function endBezierDrag(stateControl: StateControl): boolean {
    const { transientState, state, upd } = stateControl;
    const { dragState } = transientState;
    if (dragState.bezierIdx === undefined) { return false; }
