...
function clearMarkers(stateControl: StateControl): void {
    const markers = stateControl.transientState.arcClickMarkers;
    if (markers) { for (const m of markers) { m.remove(); } markers.length = 0; }
}
...
function onAddSpecialClick(
        ...
        addSpecial: { kind: 'arcFromQuads' | 'arcFromCubics'; clicks: number[][] },
        p: number[]): void {
