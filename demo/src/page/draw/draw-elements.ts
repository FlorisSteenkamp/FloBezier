...
function drawElements(
    ...
    toDraws.boundingHull && $svgs.boundingHull.push(...hulls.filter((h): h is number[][] => !!h).map(hull => drawFs.polygon(g, hull, 'lightgray thin10 nofill')));

    toDraws.quadraticToPolyline && $svgs.quadraticToPolyline.push(
        ...beziers.map(ps => splitByDeviationFromStraighLine(ps, controlPointRadius(viewbox))
        .map(t => drawFs.circle(
            g, { center: evalDeCasteljau(ps, t), radius: controlPointRadius(viewbox) * 0.5 },
            'yellow thin0'
        )[0]))
    );

    toDraws.polyQuads && $svgs.polyQuads.push(
        ...beziers.filter(ps => ps.length === 4)
        .map(ps => fitQuadsToCubic(ps, controlPointRadius(viewbox)/10)
        .map((quad, i) => drawFs.bezier(
            g, quad, `${i % 2 === 0 ? 'hueA' : 'hueB'} thin100 nofill fade`
        )[0]))
    );
    ...
    toDraws.curvatureMaxima && $svgs.curvatureMaxima.push(
        ...beziers
    ...
    if (toDraws.curvature && beziers.length) {
        ...
        beziers.forEach(ps => {
            ...
            const arr: { p: number[]; n: number[]; k: number }[] = [];
            ...
            $svgs.curvature.push(
                arr.map(s => drawFs.line(
                    g, [s.p, [s.p[0] + s.n[0] * s.k * scale, s.p[1] + s.n[1] * s.k * scale]], 'brown thin10'
                )[0])
            );
        });
    }
    ...
    if ((toDraws.normal || toDraws.tangent) && transientState.mouseXY && beziers.length) {
        ...
        let best: { ps: number[][]; t: number; at: number[] } | undefined;
        let minD = Infinity;
        for (const ps of beziers) {
            const cp = closestPointOnBezier(ps, p);
            if (cp.d < minD) { minD = cp.d; best = { ps, t: cp.t, at: cp.p }; }
        }
...
function drawArrow(g: SVGGElement, from: number[], to: number[], klass: string): SVGElement[] {
