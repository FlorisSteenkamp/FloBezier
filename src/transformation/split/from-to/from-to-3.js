...
exports.fromTo3 = void 0;
var eps = Number.EPSILON;
var u = eps / 2;
var abs = Math.abs;
/** error free error bounds */
var psErrorFree = [[0, 0], [0, 0], [0, 0], [0, 0]];
/**
...
function splitRight3(ps, t) {
    ...
    var ttt = t * tt; // <2>ttt <= <0>t<1>tt  (again by counter rule 2)
    var ss = s * s; // <1>ss  <= <0>s<0>s
    var sss = s * ss; // <2>sss <= <0>s<1>ss
    var ts = t * s; // <1>ts  <= <0>t<0>s
    var sst = t * ss; // <2>sst <= <0>t<1>ss
    var stt = s * tt; // <2>stt <= <0>s<1>tt
    var psR = [
    ...
    var _xx2 = _x3 * t + _x2 * s;
    // <4>xx1 <= <4>(<3>(<2>(<0>x3*<1>tt) + <2>(<0>x1*<1>ss)) + <2>(2*<0>x2*<1>ts))
    var _xx1 = (_x3 * tt + _x1 * ss) + 2 * _x2 * ts;
    // <6>xx0 <= <6>(<4>(<3>(<0>x3*<2>ttt) + <3>(<0>x0*<2>sss)) + <5>(3*(<4>(<3>(<0>x2*<2>stt) + <3>(<0>x1*<2>sst)))))
    var _xx0 = (_x3 * ttt + _x0 * sss) + 3 * (_x2 * stt + _x1 * sst);
    var _yy2 = _y3 * t + _y2 * s;
    var _yy1 = (_y3 * tt + _y1 * ss) + 2 * _y2 * ts;
    var _yy0 = (_y3 * ttt + _y0 * sss) + 3 * (_y2 * stt + _y1 * sst);
...
function splitAtBoth3(ps, t, tE) {
    ...
    var v = ((tE - t) / s) * (1 + Number.EPSILON) + 1 - 1;
    var tt = t * t; // <1>tt  <= <0>t<0>t   (by counter rule 2)
    var ttt = t * tt; // <2>ttt <= <0>t<1>tt  (again by counter rule 2)
    var ss = s * s; // <1>ss  <= <0>s<0>s
    var sss = s * ss; // <2>sss <= <0>s<1>ss
    var ts = t * s; // <1>ts  <= <0>t<0>s
    var sst = ss * t; // <2>sst <= <0>t<1>ss
    var tts = tt * s; // <2>stt <= <0>s<1>tt
    // splitLeft
    var sL = 1 - v; // <0>s <= exact by precondition 3 and by construction that `eps | v`
    var ttL = v * v; // <1>uu <= <0>u<0>u
    var tttL = v * ttL; // <2>uuu <= <0>u<1>uu
    var ssL = sL * sL; // <1>ss  <= <0>s<0>s
    var sssL = sL * ssL; // <2>sss <= <0>s<1>ss
    var tsL = v * sL; // <1>us  <= <0>u<0>s
    var ssuL = ssL * v; // <2>ssu  <= <1>ss<0>u
    var xa = (ss * x1 + tt * x3) + 2 * ts * x2;
    var xb = ttL * (s * x2 + t * x3);
    var xx0 = 3 * (tts * x2 + sst * x1) + (sss * x0 + ttt * x3);
    var xx1 = sL * xx0 + v * xa;
    var xx2 = (ssL * xx0 + xb) + tsL * 2 * xa;
    var xx3 = 3 * (ssuL * xa + sL * xb) + (sssL * xx0 + tttL * x3);
    var ya = ss * y1 + tt * y3 + 2 * ts * y2;
    var yb = ttL * (s * y2 + t * y3);
    var yy0 = 3 * (tts * y2 + sst * y1) + (sss * y0 + ttt * y3);
    var yy1 = sL * yy0 + v * ya;
    var yy2 = (ssL * yy0 + yb) + tsL * 2 * ya;
    var yy3 = 3 * (ssuL * ya + sL * yb) + (sssL * yy0 + tttL * y3);
    // -----------------------
    // Calculate error bounds
    // -----------------------
    var _x0 = abs(x0);
    ...
    var _xa = (ss * _x1 + tt * _x3) + 2 * ts * _x2;
    // <4>xb <= <4>(<1>ttL*<2>((<1>(<0>s*<0>x2) + <1>(<0>t*<0>x3))));
    var _xb = ttL * (s * _x2 + t * _x3);
    // <6>xx0 <= <6>(<5>(3*<4>(<3>(<2>tts*<0>x2) + <3>(<2>sst*<0>x1))) + <4>(<3>(<2>sss*<0>x0) + <3>(<2>ttt*<0>x3)));
    var _xx0 = 3 * (tts * _x2 + sst * _x1) + (sss * _x0 + ttt * _x3);
    // <6>xx1 = <6>(<1>(<0>sL*<0>xx0) + <5>(<0>u*<4>xa));
    var _xx1 = sL * _xx0 + v * _xa;
    // <10>xx2 = <10>(<9>(<8>(<1>ssL*<6>xx0) + <4>xb) + <6>(<1>tsL*2*<4>xa));
    var _xx2 = (ssL * _xx0 + _xb) + tsL * 2 * _xa;
    // <11>xx3 = <11>(<9>(3*<8>(<7>(<2>ssuL*<4>xa) + <5>(<0>sL*<4>xb))) + <10>(<9>(<2>sssL*<6>xx0) + <3>(<2>tttL*<0>x3)));
    var _xx3 = 3 * (ssuL * _xa + sL * _xb) + (sssL * _xx0 + tttL * _x3);
    var _ya = ss * _y1 + tt * _y3 + 2 * ts * _y2;
    var _yb = ttL * (s * _y2 + t * _y3);
    var _yy0 = 3 * (tts * _y2 + sst * _y1) + (sss * _y0 + ttt * _y3);
    var _yy1 = sL * _yy0 + v * _ya;
    var _yy2 = (ssL * _yy0 + _yb) + tsL * 2 * _ya;
    var _yy3 = 3 * (ssuL * _ya + sL * _yb) + (sssL * _yy0 + tttL * _y3);
    return {
