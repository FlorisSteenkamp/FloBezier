...
function getTransformation(
    ...
    return ab !== undefined 
        ? ab 
        : getLinearTransformation(xy_[1], xy[1])!;
}
 ...
 * @param r the required output polynomial
 * 
 * @internal
 */
/*
function getLinearTransformation(
        p: number[][],
        r: number[][]): { a: number[], b: number[] } | undefined {

    const [p0,p1,p2,p3] = p;
    const [r0,r1,r2,r3] = r;

    if (eSign(p0) === 0) { // (then `r0` will also be `0`)
        // it is not possible to find `a` with this coordinate
        // - use the other coordinate (that is guaranteed to have `p0 !== 0`)
        return undefined; 
    }
