...
def getCurvatureExtremaPoly_cubic():
    # constants
    ...
    kk = ((dddx*dyt - dxt*dddy)*(dxt*dxt + dyt*dyt) - 3*(dxt*ddxt + dyt*ddyt)*(ddxt*dyt - dxt*ddyt))
    ...
    #kk = kk.subs(dddx, 6*(x3 + 3*(x1 - x2) - x0))
    ...
    #kk = kk.subs(dddy, 6*(y3 + 3*(y1 - y2) - y0))

    print(collect(expand(kk),t))

getCurvatureExtremaPoly_cubic()
