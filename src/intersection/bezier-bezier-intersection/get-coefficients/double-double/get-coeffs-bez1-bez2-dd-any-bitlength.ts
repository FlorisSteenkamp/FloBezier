 ...
 * @doc mdx
 */
function getCoeffsBez1Bez2DdAnyBitlength(ps1: number[][], ps2: number[][]) {
    const {
        coeffs: { vₓ, vᵧ, v },
        errorBound: { v_ } 
    } = getImplicitForm1DdAnyBitlength(ps1);

    const {
        coeffs: [[c2,c1,c0],[d2,d1,d0]],
        errorBound: [[c2_],[d2_]]
    } = getXYDdAnyBitlength2(ps2);
...
export { getCoeffsBez1Bez2DdAnyBitlength }
