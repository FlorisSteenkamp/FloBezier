...
function getCoeffsCubicQuad(
    ...
    let [[a3,a2,a1,a0],[b3,b2,b1,b0]] = getXY(ps);  // exact if bitlength <= 49
 ...
 */
function getCoeffsQuadraticQuad(
        circle: { center: number[], radius: number}, 
        ps: number[][]): number[][] {

    let { radius: r, center: [cx, cy] } = circle;
    let [[a2,a1,a0],[b2,b1,b0]] = getXY(ps);  // exact if bitlength <= 49
 ...
 */
function getCoeffsLinearQuad(
        circle: { center: number[], radius: number}, 
        ps: number[][]): number[][] {

    let { radius: r, center: [cx, cy] } = circle;
    let [[a1,a0],[b1,b0]] = getXY(ps);  // exact if bitlength <= 49
