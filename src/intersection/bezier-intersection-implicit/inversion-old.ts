...
import { 
    twoProduct, calculateSum, estimate, expansionDiv, expansionProduct, 
    expansionDiff, negativeOf, scaleExpansion, fastExpansionSum, twoSum, twoDiff, sign, compare, abs 
} from "flo-numerical";
...
function inversion3(ps: number[][], p: number[][]) {
    ...
    if (sign(t22) !== 0 && sign(t12) !== 0) {
        let d = expansionDiv(t12, t22, 4);
        a1 = expansionDiff(t11, expansionProduct(d, t21));
        b1 = expansionDiff(t10, expansionProduct(d, t20));
    }

    // eliminate the coefficient of t^2 from the second row (using the 3rd)
    if (sign(t32) !== 0 && sign(t22) !== 0) {
        let d = expansionDiv(t22, t32, 4);
        a2 = expansionDiff(t21, expansionProduct(d, t31));
        b2 = expansionDiff(t20, expansionProduct(d, t30));
    }

    // eliminate the coefficient of t^2 from the third row (using the 1st)
    if (sign(t12) !== 0 && sign(t32) !== 0) {
        let d = expansionDiv(t32, t12, 4);
        a3 = expansionDiff(t31, expansionProduct(d, t11));
        b3 = expansionDiff(t30, expansionProduct(d, t10));
    }
    ...
    a2 = a2 || [0];
    a3 = a3 || [0];
    if (compare(abs(a1),abs(a2)) > 0) {
        if (compare(abs(a1),abs(a3)) > 0) {
            useRow = 1;
        } else {
            useRow = 3;
        }
    } else {
        if (compare(abs(a2),abs(a3)) > 0) {
            useRow = 2;
        } else {
            useRow = 3;
        }
    ...
    let t = expansionDiv(negativeOf(b), a, 4);
    return estimate(t);
}
...
function inversion2(ps: number[][], p: number[][]) {
    ...
    let t = expansionDiv(negativeOf(t10), t11, 4);
    return estimate(t);
