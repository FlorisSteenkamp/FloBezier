...
exports.getImplicitForm3_bitlength16_double = void 0;
 ...
 * @param ps
 */
function getImplicitForm3_bitlength16_double(ps) {
    // The implicit form is given by:
    // vₓₓₓx³ + vₓₓᵧx²y + vₓᵧᵧxy² + vᵧᵧᵧy³ + vₓₓx² +vₓᵧxy + vᵧᵧy² + vₓx + vᵧy + v = 0
    // Bitlength increases = [3,4,3,0]
    let [[a3, a2, a1, a0], [b3, b2, b1, b0]] = get_xy_1.getXY(ps);
    let a3b1 = a3 * b1; // Max bitlength = 39 = (16+4) + (16+3)
    let a1b3 = a1 * b3;
    let a3b2 = a3 * b2;
    let a2b2 = a2 * b2;
    let a2b3 = a2 * b3;
    let a3a3 = a3 * a3; // Max bitlength = 40 = (16+4) + (16+4)
    let b2b2 = b2 * b2;
    let b3b3 = b3 * b3;
    let a1a3 = a1 * a3;
    let a2a2 = a2 * a2;
    ...
    let y1: number;
    let y0: number;
    let a =  + 3*(y0*(x1 - x2) - x0*(y1 + y2)) + x3*y0 - x0*y3
    // 36 bits
    */
    let q1 = a3b0 - a0b3; // Max bitlength = 41 = 40 + 1 ??
    let q2 = a3b1 - a1b3;
    let q3 = a3b2 - a2b3;
    let q4 = a2b0 - a0b2;
    let q5 = a2b1 - a1b2;
    ...
    let _p2_ = abs(p2);
    let _tq2_ = 2 * _q2_;
    let q1q1 = q1 * q1; // Max bitlength = 82 = 41 + 41
    let q1q2 = q1 * q2;
    let q1q3 = q1 * q3;
    let q1q5 = q1 * q5;
    let q2q2 = q2 * q2;
    ...
    let q3q5 = q3 * q5;
    let q3q6 = q3 * q6;
    let q1q1_ = 2 * (_q1_ * _q1_) + abs(q1q1);
    let q1q2_ = _q1_ * _q2_ + _q1_ * _q2_ + abs(q1q2);
    let q1q3_ = _q1_ * _q3_ + _q1_ * _q3_ + abs(q1q3);
    let q1q5_ = _q1_ * _q5_ + _q1_ * _q5_ + abs(q1q5);
    let q2q2_ = 2 * (_q2_ * _q2_) + abs(q2q2);
    let tq2q4_ = _tq2_ * _q4_ + _tq2_ * _q4_ + abs(tq2q4);
    let q3q4_ = _q3_ * _q4_ + _q3_ * _q4_ + abs(q3q4);
    let q3q5_ = _q3_ * _q5_ + _q3_ * _q5_ + abs(q3q5);
    let q3q6_ = _q3_ * _q6_ + _q3_ * _q6_ + abs(q3q4);
    let u1 = -3 * q1 - q5;
    let _u1 = abs(u1);
    let u1_ = 3 * (_q1_ + _q1_) + _q5_ + _u1;
    let vₓₓₓ = -b3 * b3b3;
    ...
    let w2 = q3 * t1;
    let w2_ = _q3_ * _t1 + t1_ * _q3_ + abs(w2);
    let w3 = w1 + w2;
    let w3_ = w1_ + w2_ + abs(w3);
    let w4 = tq2 * b2b3;
    let w4_ = _b2b3 * (_tq2_ + _tq2_) + abs(w4);
    ...
    let w6 = q3 * t2;
    let w6_ = t2_ * _q3_ + _q3_ * _t2 + abs(w6);
    let w7 = w5 + w6;
    let w7_ = w5_ + w6_ + abs(w7);
    let w8 = tq2 * a2a3;
    let w8_ = _a2a3 * (_tq2_ + _tq2_) + abs(w8);
    //let vᵧᵧ = (u1*a3a3 + q3*t2) + tq2*a2a3;
    let vᵧᵧ = w7 + w8;
    let vᵧᵧ_ = w7_ + w8_ + abs(vᵧᵧ);
    let wa = a2b2 - p2 / 2;
    let _wa = abs(wa);
    let wa_ = _a2b2 + _p2_ / 2 + _wa;
    let wb = u1 * a3b3;
    let wb_ = _a3b3 * (u1_ + _u1) + abs(wb);
    let wc = q2 * p1;
    let wc_ = _q2_ * _p1_ + _q2_ * _p1_ + abs(wc);
    let wd = wb + wc;
    let wd_ = wb_ + wc_ + abs(wd);
    let wq = q3 * wa;
    let wq_ = _q3_ * _wa + _q3_ * wa_ + abs(wq);
    ...
    let v4 = s3 * q6;
    let v4_ = s3_ * abs(q6) + abs(s3) * _q6_ + abs(v4);
    let v5 = q3q4 * q4;
    let v5_ = q3q4_ * _q4_ + abs(q3q4) * _q4_ + abs(v5);
    let v2 = v4 - v5;
    let v2_ = v4_ + v5_ + abs(v2);
    let v6 = q1 * v1;
    let v6_ = _q1_ * _v1 + _q1_ * v1_ + abs(v6);
...
exports.getImplicitForm3_bitlength16_double = getImplicitForm3_bitlength16_double;
//# sourceMappingURL=get-implicit-form3-bitlength16-double.js.map