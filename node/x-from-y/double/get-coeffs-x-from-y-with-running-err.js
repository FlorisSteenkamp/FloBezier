import { getCoeffsYFromX_WithRunningErr } from "../../y-from-x/double/get-coeffs-y-from-x-with-running-err.js";
/**
 ...
 */
function getCoeffsXFromY_WithRunningErr(ps, y) {
    // swap the x and y coordinates (reflect in the line y = x)
    const psₛ = ps.map(p => [p[1], p[0]]);
    return getCoeffsYFromX_WithRunningErr(psₛ, y);
}
export { getCoeffsXFromY_WithRunningErr };
//# sourceMappingURL=get-coeffs-x-from-y-with-running-err.js.map