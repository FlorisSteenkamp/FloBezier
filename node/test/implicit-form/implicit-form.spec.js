...
import { getImplicitForm1, getImplicitForm2, getImplicitForm3, getImplicitForm1DdWithRunningError, getImplicitForm2DdWithRunningError, getImplicitForm3DdWithRunningError, getImplicitForm1Dd, getImplicitForm2Dd, getImplicitForm3Dd, getImplicitForm1Exact, getImplicitForm2Exact, getImplicitForm3Exact, getImplicitForm1ErrorCounters, getImplicitForm2ErrorCounters, getImplicitForm3ErrorCounters, } from '../../src/index.js';
...
const γ1 = γ(1);
const γγ3 = γγ(3);
function getImplicitForm1Error(ps) {
...
function getImplicitForm2Error(ps) {
...
function getImplicitForm3Error(ps) {
...
const _bz_3 = [[0, 0], [0, 0], [0, 0], [0, 0]];
const _bz_2 = [[0, 0], [0, 0], [0, 0]];
const _bz_1 = [[0, 0], [0, 0]];
const implFormFs = [,
...
describe('implicit form', function () {
    it('it should find the implicit form (including error bound) of some beziers in double precision', function () {
        let i = 0;
        let j = 0;
        while (i < num) {
    ...
    it('it should find the implicit form (including error bound) of some beziers in double-double precision', function () {
        let i = 0;
        let j = 0;
        while (i < num) {
            // get some random beziers
    ...
    it('it should find the implicit form (including error bound) of some specific beziers in double-double precision', function () {
        const a = 2 ** 0;
        const b = 2 ** 0;
        const bzs = [];
        const bz1 = [
        ...
        const bz2 = [
            [0.4618254473553094 + a - a, 3.1 + b - b],
...
function testImplictForm(isDd, f, fExact, bz, fExclErr) {
    const implEst = f(bz);
    ...
    for (const key_ in implEst.coeffs) {
        ...
        const err = implEst.errorBound[key + '_'];
        const errBound = isDd
            ? γγ3 * (err || 0)
            : γ1 * (err || 0);
        const errActual = Math.abs(eEstimate(eDiff(rExact, rEst)));
        const errStr = `
            --------
            dd        : ${isDd}
            key       : ${key}
...
//# sourceMappingURL=implicit-form.spec.js.map