...
const _bz_3 = [[0, 0], [0, 0], [0, 0], [0, 0]]; // dummy cubic
const _bz_2 = [[0, 0], [0, 0], [0, 0]]; // dummy quadratic
const _bz_1 = [[0, 0], [0, 0]]; // dummy line
const implFormDdFs = [,
    [,
...
describe('intersection coefficients', function () {
    it('it should find valid polynomial intersection coefficients (including error bound) of beziers in double-double precision (and for Shewchuk expansions)', function () {
        let i = 0;
        while (i++ < numTests) {
            let j = 5;
...
function testCoeffsDd(j, k, f, fExact, bz1, bz2, iteration) {
    let est = f(bz1, bz2);
    ...
    for (let i = 0; i < j * k; i++) {
        ...
        let errActual = abs(eEstimate(eDiff(rExact, rEst)));
        let errStr = `
        --------
        bez-orders : ${j}x${k}
        indx       : ${i}
...
//# sourceMappingURL=coeffs.spec.js.map