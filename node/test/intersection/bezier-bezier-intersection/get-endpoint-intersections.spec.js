...
import { evaluateExact, generateSelfIntersecting } from "../../../src/index.js";
...
describe('getEndpointIntersections', function () {
    it('it should find intersection intervals pairs of overlapping/non-overlapping algebraically identical bezier curves', function () {
        const psCubic = [
            [[-1, 1], [6, 2.5], [2, 1], [3, 3]],
            [[1, 1], [6, -2.5], [2, 1], [3, 3]],
            [[1, 1], [6, 2.5], [2, 1], [-3, -3]],
...
function testEndpointXs(t0, t1, psA, expectedA, expectedB) {
    const psB = psA.length === 4
    ...
    for (let i = 0; i < 2; i++) {
        ...
        // ⇒ ⇒
        {
            ...
            for (let j = 0; j < r.length; j++) {
                expect(
                // @ts-ignore - otherwise TypeScript gives an error on nearly
                evaluateExact(B, expectedB_[j]).map(eEstimate)).to.be.nearly(2 ** 4, evaluateExact(A, expectedA_[j]).map(eEstimate));
            }
        }
        // ⇐ ⇒  (A reversed)
        {
            ...
            for (let j = 0; j < r.length; j++) {
                expect(
                // @ts-ignore - otherwise TypeScript gives an error on nearly
                evaluateExact(B, expectedBR[j]).map(eEstimate)).to.be.nearly(2 ** 4, evaluateExact(AR, expectedAR[j]).map(eEstimate));
            }
        }
        // ⇒ ⇐ (B reversed)
        {
            ...
            for (let j = 0; j < r.length; j++) {
                expect(
                // @ts-ignore - otherwise TypeScript gives an error on nearly
                evaluateExact(BR, expectedBR[j]).map(eEstimate)).to.be.nearly(2 ** 4, evaluateExact(A, expectedAR[j]).map(eEstimate));
            }
        }
        // ⇐ ⇐ (A and B reversed)
        {
            ...
            for (let j = 0; j < r.length; j++) {
                expect(
                // @ts-ignore - otherwise TypeScript gives an error on nearly
                evaluateExact(BR, expectedBR[j]).map(eEstimate)).to.be.nearly(2 ** 4, evaluateExact(AR, expectedAR[j]).map(eEstimate));
            }
        }
    }
}
...
*/ 
//# sourceMappingURL=get-endpoint-intersections.spec.js.map