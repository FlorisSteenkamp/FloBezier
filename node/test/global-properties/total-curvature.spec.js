...
import { evaluateDdxy, evaluateDxy, getInterfaceRotation, splitByCurvature, tangent, totalAbsoluteCurvature, totalCurvature, generateSelfIntersecting, fromTo, generateCuspAtHalf3 } from '../../src/index.js';
...
const 𝜋 = Math.PI;
function totalCurvatureByGauss(ps, interval, flatness, gaussOrder) {
    if (ps.length === 1) {
        return 0;
    }
    ...
    for (let i = 0; i < ts.length - 1; i++) {
        ...
        total += gaussQuadrature(κds(ps_), interval, gaussOrder);
    }
    return total;
}
function κds(ps) {
...
function test(ps) {
    {
        ...
        assert(closeTo([ulpsOrEps])(c - d, 0), 
...
function gaussCompare() {
    ...
    {
        const pss = [];
        pss.push([[0, 0], [100, 0], [90, 30], [50, -100]], [
    ...
    for (let order = 0; order <= 3; order++) {
        ...
        const s = 0;
        for (let i = 0; i < len; i++) {
            const ps = getRandomBezier_(order)(i + (s * len));
            test(ps);
        }
    }
    {
        ...
        for (let i = 2; i < len; i++) {
            ...
            const ps1 = generateSelfIntersecting(ps[0], ps[1], ps[2], [abs(ps[3][0]), abs(ps[3][1])]);
            const ps2 = generateSelfIntersecting(ps[0], ps[1], ps[2], ps1[3].slice().reverse());
            test(ps1);
            test(ps2);
        }
    }
}
function testBeziersInLoop(order) {
...
describe('totalCurvature', function () {
    it('it should calculate total curvature roughly equal to that calculated with Gauss Quadrature for some bezier curves', function () {
        gaussCompare();
    });
    it('it should calculate total curvature a multiple of 2𝜋 if some bezier curves form a closed loop', function () {
        testBeziersInLoop(1);
        testBeziersInLoop(2);
        testBeziersInLoop(3);
    });
    it('it should calculate the correct total curvature for some bezier curves', function () {
        testSomeTotalCurvature();
    });
});
describe('totalAbsoluteCurvature', function () {
    it('it should calculate the correct total absolute curvature for some bezier curves', function () {
        {
...
//# sourceMappingURL=total-curvature.spec.js.map