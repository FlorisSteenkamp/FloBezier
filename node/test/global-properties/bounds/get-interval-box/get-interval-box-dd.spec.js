...
import { eDiff, eEstimate, eSign } from 'big-float-ts';
...
describe('getIntervalBoxDd', function () {
    it('it should get a reasonable interval box in double-double precision for some lines', function () {
        {
    ...
    it('it should get a reasonable interval box in double-double precision for some quadratic beziers', function () {
        {
    ...
    it('it should get a reasonable interval box in double-double precision for some cubic beziers', function () {
        {
    ...
    it('it should get a reasonable interval box in double-double precision for special cases', function () {
        {
            // ts = [0,0]
            const ps = [[3.3, -1.3], [2.3, -1.3], [1.3, 1.3], [0.1, 0.1]];
            const tS = [0, 0];
 ...
 * @param box
 */
function isPointInBoxE(p, box) {
    return (eSign(eDiff(p[0], box[0][0])) >= 0 &&
        eSign(eDiff(box[1][0], p[0])) >= 0 &&
        eSign(eDiff(p[1], box[0][1])) >= 0 &&
        eSign(eDiff(box[1][1], p[1])) >= 0);
}
function evaluateExactE(ps, t) {
...
//# sourceMappingURL=get-interval-box-dd.spec.js.map