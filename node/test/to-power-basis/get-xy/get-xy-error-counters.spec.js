...
import { γ, getXYExact, getXYErrorCounters, getXY } from '../../../src/index.js';
...
describe('getXYWithRunningError', function () {
    it('it should get the the power basis representation error bound counters of some bezier curves (in double precision)', function () {
        {
            let k = 0;
            for (let i = 0; i <= 3; i++) {
                for (let j = 0; j < 25; j++) {
                    ...
                    //----------------------------------------------------------------------
                    const xy = getXY(ps);
                    const rD = mapDoubleToShewchuk(xy);
...
//# sourceMappingURL=get-xy-error-counters.spec.js.map