...
describe('getXYDdWithRunningError', function () {
    it('it should get the the power basis representation (and a running error bound) of some bezier curves (in double-double precision)', function () {
        {
            let k = 0;
            for (let i = 0; i <= 3; i++) {
                for (let j = 0; j < 25; j++) {
                    ...
                    const errorBound = _errorBound.map((err) => err.map(e => γγ3 * e));
                    const rE = getXYExact(ps);
                    const errorActual = mapShewchukToAbsDouble(subtractShewchuk(rDd, rE));
...
//# sourceMappingURL=get-xy-dd-with-running-error.spec.js.map