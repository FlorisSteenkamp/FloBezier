...
describe('getXYWithRunningError', function () {
    it('it should get the the power basis representation (and a running error bound) of some bezier curves (in double precision)', function () {
        {
            let k = 0;
            for (let i = 0; i <= 3; i++) {
                for (let j = 0; j < 25; j++) {
                    const _ps = getRandomBezier_(i)(j);
                    ...
                    const errorBound = _errorBound.map((err) => err.map(e => γ1 * e));
                    const rD = mapDoubleToShewchuk(coeffs);
                    const rE = getXYExact(ps);
                    const errorActual = mapShewchukToAbsDouble(subtractShewchuk(rD, rE));
...
//# sourceMappingURL=get-xy-with-running-error.spec.js.map