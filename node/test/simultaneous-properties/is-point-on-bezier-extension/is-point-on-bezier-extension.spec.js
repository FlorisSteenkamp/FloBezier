...
describe('isPointOnBezierExtension', function () {
    it('it should correctly check if some given points are exactly on some given bezier curves', function () {
        for (let order = 0; order <= 3; order++) {
            for (let seed = 0; seed < 25; seed++) {
                const { ps, p, t } = getPointExactlyOnCurve(order, seed);
                const p1 = p.map(c => [c]);
...
//# sourceMappingURL=is-point-on-bezier-extension.spec.js.map