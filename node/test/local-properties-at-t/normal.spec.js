...
describe('normal', function () {
    it('it should accurately calculate the normal of some bezier curves at some `t` values', function () {
        for (let i = 0; i < 10; i++) {
            for (let order = 1; order <= 3; order++) {
                const ps = getRandomBezier(128, 53)(order)(i);
                let ts = [0, 0.3, 0.9, 1];
                for (let t of ts) {
                    let r = normal(ps, t);
                    let s = tangent(ps, t);
...
//# sourceMappingURL=normal.spec.js.map