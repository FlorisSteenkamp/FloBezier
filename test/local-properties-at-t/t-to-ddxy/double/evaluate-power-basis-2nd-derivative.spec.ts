...
import { 
    evalDeCasteljau, evaluatePowerBasis_2ndDerivative, evaluatePowerBasis_1stDerivative, 
    evaluatePowerBasis_2ndDerivativeAt0, evaluatePowerBasis_2ndDerivativeAt1, 
    evaluatePowerBasis_1stDerivativeAt0, evaluatePowerBasis_1stDerivativeAt1, 
    evaluatePowerBasis_2ndDerivativeAt0Exact, evaluatePowerBasis_2ndDerivativeAt1Exact, 
    evaluatePowerBasis_1stDerivativeAt0Exact, evaluatePowerBasis_1stDerivativeAt1Exact
} from '../../../../src/index.js';
...
describe('evaluatePowerBasis_2ndDerivative', function() {
	...
	function() {
        {
            ...
            const dxy = evaluatePowerBasis_1stDerivative(ps,t);
            // @ts-ignore - otherwise TypeScript gives an error on nearly
            expect(dxy).to.be.nearly(2**34, dxyEst);

            const ddxyEst = ddxyByForwardDifferences(ps,t,δt);
            const ddxy = evaluatePowerBasis_2ndDerivative(ps,t);
            // @ts-ignore - otherwise TypeScript gives an error on nearly
...

            {
                ...
                const dxy = evaluatePowerBasis_1stDerivativeAt0(ps);
                // @ts-ignore - otherwise TypeScript gives an error on nearly
                expect(dxy).to.be.nearly(2**36, dxyEst);

                const ddxyEst = ddxyByForwardDifferences(ps,0,δt);
                const ddxy = evaluatePowerBasis_2ndDerivativeAt0(ps);
                // @ts-ignore - otherwise TypeScript gives an error on nearly
                expect(ddxy).to.be.nearly(2**42,ddxyEst);
            }
            {
                ...
                const dxy = evaluatePowerBasis_1stDerivativeAt1(ps);
                // @ts-ignore - otherwise TypeScript gives an error on nearly
                expect(dxy).to.be.nearly(2**34, dxyEst);

                const ddxyEst = ddxyByForwardDifferences(ps,1,δt);
                const ddxy = evaluatePowerBasis_2ndDerivativeAt1(ps);
                // @ts-ignore - otherwise TypeScript gives an error on nearly
                expect(ddxy).to.be.nearly(2**42,ddxyEst);
            }
        }
...

        {
            ...
            expect(() => evaluatePowerBasis_2ndDerivativeAt0(bez4)).to.throw();
            expect(() => evaluatePowerBasis_2ndDerivativeAt1(bez4)).to.throw();
            expect(() => evaluatePowerBasis_2ndDerivativeAt0Exact(bez4)).to.throw();
            expect(() => evaluatePowerBasis_2ndDerivativeAt1Exact(bez4)).to.throw();

            expect(() => evaluatePowerBasis_1stDerivativeAt0(bez4)).to.throw();
            expect(() => evaluatePowerBasis_1stDerivativeAt1(bez4)).to.throw();
            expect(() => evaluatePowerBasis_1stDerivativeAt0Exact(bez4)).to.throw();
            expect(() => evaluatePowerBasis_1stDerivativeAt1Exact(bez4)).to.throw();

            const p1 = [[1,1]];
            expect(evaluatePowerBasis_1stDerivativeAt0Exact(p1)).to.eql([[0],[0]]);
            expect(evaluatePowerBasis_1stDerivativeAt1Exact(p1)).to.eql([[0],[0]]);
        }
	});
});
