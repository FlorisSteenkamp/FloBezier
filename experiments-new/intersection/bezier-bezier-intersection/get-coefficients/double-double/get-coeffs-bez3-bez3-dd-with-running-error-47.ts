import { getImplicitForm3DdWithRunningError47 } from "../../../../implicit-form/double-double/get-implicit-form3-dd-with-running-error-47";
 ...
 * @doc mdx
 */
function getCoeffsBez3Bez3DdWithRunningError47(ps1: number[][], ps2: number[][]) {
    const { 
        coeffs: { vₓₓₓ, vₓₓᵧ, vₓᵧᵧ, vᵧᵧᵧ, vₓₓ, vₓᵧ, vᵧᵧ, vₓ, vᵧ, v },
        errorBound: { vₓₓₓ_, vₓₓᵧ_, vₓᵧᵧ_, vᵧᵧᵧ_, vₓₓ_, vₓᵧ_, vᵧᵧ_, vₓ_, vᵧ_, v_ }
    } = getImplicitForm3DdWithRunningError47(ps1);
    ...
    //    (d3*d3d3)*vᵧᵧᵧ;  
    const $g1 = c3*$c3c3;
    const g1 = qmd(c3,c3c3);
    const _g1 = _c3*_c3c3;
    const $g2 = c3*$d3d3;
    const g2 = qmd(c3,d3d3);
    const _g2 = _c3*_d3d3;
    const $g3 = d3*$c3c3;
    const g3 = qmd(d3,c3c3);
    const _g3 = _d3*_c3c3;
    const $g4 = d3*$d3d3;
    const g4 = qmd(d3,d3d3);
    const _g4 = _d3*_d3d3;
    const $g5 = $g1*$vₓₓₓ;
    const g5 = qmq(g1,vₓₓₓ);
    const g5_ = _g1*(_vₓₓₓ + vₓₓₓ_) + 2*abs($g5);
    const $g6 = $g2*$vₓᵧᵧ;
    const g6 = qmq(g2,vₓᵧᵧ);
    const g6_ = _g2*(_vₓᵧᵧ + vₓᵧᵧ_) + 2*abs($g6);
    const $g7 = $g3*$vₓₓᵧ;
    const g7 = qmq(g3,vₓₓᵧ);
    const g7_ = _g3*(_vₓₓᵧ + vₓₓᵧ_) + 2*abs($g7);
    const $g8 = $g4*$vᵧᵧᵧ;
    const g8 = qmq(g4,vᵧᵧᵧ);
    const g8_ = _g4*(_vᵧᵧᵧ + vᵧᵧᵧ_) + 2*abs($g8);
    const $g9 = $g5 + $g6;
    const g9 = qaq(g5,g6);
    ...
    const w4_ = abs($w4);
    const $w5 = c2*$c3c3;
    const w5 = qmd(c2,c3c3);
    //const _w5 = abs($w5);
    const w5_ = abs($w5);
    const $w6 = d2*$d3d3;
    const w6 = qmd(d2,d3d3);
    //const _w6 = abs($w6);
    const w6_ = abs($w6);
    const $w7 = $vₓₓₓ*$w5;
    const w7 = qmq(vₓₓₓ,w5);
    const w7_ = w5_*(vₓₓₓ_ + _vₓₓₓ) + 2*abs($w7);
    const $u1 = $vᵧᵧᵧ*$w6;
    const u1 = qmq(vᵧᵧᵧ,w6);
    const u1_ = w6_*(vᵧᵧᵧ_ + _vᵧᵧᵧ) + 2*abs($u1);
    const $u2 = $vₓₓᵧ*$w3;
    const u2 = qmq(vₓₓᵧ,w3);
    const u2_ = w3_*(vₓₓᵧ_ + _vₓₓᵧ) + 2*abs($u2);
    const $u3 = $vₓᵧᵧ*$w4;
    const u3 = qmq(vₓᵧᵧ,w4);
    const u3_ = w4_*(vₓᵧᵧ_ + _vₓᵧᵧ) + 2*abs($u3);
    const $u4 = $u2 + $u3;
    const u4 = qaq(u2,u3);
    ...
    //    vᵧᵧᵧ*3*d3*(d1d3 + d2d2);
    const $o1 = c1*$c3d3;
    const o1 = qmd(c1,c3d3);
    const o1_ = abs($o1);
    const $o2 = d1*$c3c3;
    const o2 = qmd(d1,c3c3);
    const o2_ = abs($o2);
    const $o3 = c2*$d2d3;
    const o3 = qmd(c2,d2d3);
    const o3_ = abs($o3);
    const $o4 = c1*$d3d3;
    const o4 = qmd(c1,d3d3);
    const o4_ = abs($o4);
    const $o5 = c2*$c3d2;
    const o5 = qmd(c2,c3d2);
    const o5_ = abs($o5);
    const $o6 = d3*$c2c2;
    const o6 = qmd(d3,c2c2);
    const o6_ = abs($o6);
    const $o7 = c3*$d1d3;
    const o7 = qmd(c3,d1d3);
    const o7_ = abs($o7);
    const $o8 = d2*$c3d2;
    const o8 = qmd(d2,c3d2);
    const o8_ = abs($o8);
    const $w8 = $o1 + $o5;
    const w8 = qaq(o1,o5);
    ...
    const wk_ = _wi_*(vₓₓₓ_ + _vₓₓₓ) + 2*abs($wk);
    const $wl = $vᵧᵧᵧ*$wj;
    const wl = qmq(vᵧᵧᵧ,wj);
    const wl_ = _wj_*(vᵧᵧᵧ_ + _vᵧᵧᵧ) + 2*abs($wl);
    const $wm = $wg + $wh;
    const wm = qaq(wg,wh);
    ...
    const _wo = abs($wo);
    const $zc = d2*$c2c2;
    const zc = qmd(d2,c2c2);
    const zc_ = abs($zc);
    const $zd = 2*c1*$wo;
    const zd = qm2(qmd(c1,wo));
    const zd_ = 2*abs($zd);
    const $wp = $zc + $zd;
    const wp = qaq(zc,zd);
    ...
    const $zf = c2*$d2d2;
    const zf = qmd(c2,d2d2);
    const zf_ = abs($zf);
    const $zg = 2*d1*$wo;
    const zg = qm2(qmd(d1,wo));
    const zg_ = 2*abs($zg);
    const $wt = $zf + $zg;
    const wt = qaq(zf,zg);
    ...
    const zm_ = _c3c3*vₓₓ_ + 2*abs($zm);
    const $zn = $vᵧᵧ*$d3d3;
    const zn = qmq(d3d3,vᵧᵧ);
    const zn_ = _d3d3*vᵧᵧ_ + 2*abs($zn);
    const $z8 = $zm + $zn;
    const z8 = qaq(zm,zn);
    const z8_ = zm_ + zn_ + abs($z8);
    const $z9 = $vₓᵧ*$c3d3;
    const z9 = qmq(c3d3,vₓᵧ);
    const z9_ = _c3d3*vₓᵧ_ + 2*abs($z9);
    const $za = $z6 + $z7;
    const za = qaq(z6,z7);
    ...
    const $k1 = c0*$wo; 
    const k1 = qmd(c0,wo);  // wo: 48-bit aligned => error free
    const k1_ = abs($k1);
    const $k2 = d0*$wo;
    const k2 = qmd(d0,wo);
    const k2_ = abs($k2);
    ...
    const $k7 = d3*$c1c1;
    const k7 = qmd(d3,c1c1);
    const k7_ = abs($k7);
    const $k8 = c3*$d1d1;
    const k8 = qmd(c3,d1d1);
    const k8_ = abs($k8);
    const $k9 = c2*$k5;
    const k9 = qmd(c2,k5);
    const k9_ = abs($k9);
    ...
    const kh_ = _c2c3*vₓₓ_ + 2*abs($kh);
    const $ki = $vᵧᵧ*$d2d3;
    const ki = qmq(d2d3,vᵧᵧ);
    const ki_ = _d2d3*vᵧᵧ_ + 2*abs($ki);
    const $kj = $kb + $k7;
    const kj = qaq(kb,k7);
    const _kj = abs($kj);
    ...
    const sa_ = abs($sa);
    const $sb = c2*$c1c1;
    const sb = qmd(c2,c1c1);
    const sb_ = abs($sb);
    const $sc = d2*$d1d1;
    const sc = qmd(d2,d1d1);
    const sc_ = abs($sc);
    const $sd = $s1 + $s3;
    const sd = qaq(s1,s3);
    ...
    const $ss = $vₓᵧ*$r1;
    const ss = qmq(vₓᵧ,r1);
    const ss_ = vₓᵧ_*_r1 + 2*abs($ss);
    const $st = $vₓₓ*$r2;
    const st = qmq(vₓₓ,r2);
    const st_ = vₓₓ_*_r2 + 2*abs($st);
    const $sq = $ss + $st;
    const sq = qaq(ss,st);
    const sq_ = ss_ + st_ + abs($sq);
    const $su = $vᵧᵧ*$r3;
    const su = qmq(vᵧᵧ,r3);
    const su_ = vᵧᵧ_*_r3 + 2*abs($su);
    const $sr = $sq + $su;
    const sr = qaq(sq,su);
    ...
    const $m7 = (3*c0)*$m5;
    const m7 = qmd(3*c0,m5);  // 3*c0: 47-bit aligned => error free
    const m7_ = abs($m7);
    const $m8 = (3*d0)*$m6;  
    const m8 = qmd(3*d0,m6);  // 3*c0: 47-bit aligned => error free
    const m8_ = abs($m8);
    const $m9 = c1*$c1c1;
    const m9 = qmd(c1,c1c1);
    const m9_ = abs($m9);
    const $ma = d1*$d1d1;
    const ma = qmd(d1,d1d1);
    const ma_ = abs($ma);
    const $mb = $vₓₓ*$r8;
    const mb = qmq(vₓₓ,r8);
    const mb_ = vₓₓ_*_r8 + abs($mb);
    const $mc = $vᵧᵧ*$r9;
    const mc = qmq(vᵧᵧ,r9);
    const mc_ = vᵧᵧ_*_r9 + abs($mc);
    const $md = $m1 + $r7;
    const md = qaq(m1,r7);
    const md_ = abs($md);
    const $me = $m2 + $r7;
    ...
    const l7_ = abs($l7);
    const $l8 = c2*$c0c0;
    const l8 = qmd(c2,c0c0);
    const l8_ = abs($l8);
    const $l9 = d0*$d1d1;
    const l9 = qmd(d0,d1d1);
    const l9_ = abs($l9);
    const $la = d2*$d0d0;
    const la = qmd(d2,d0d0);
    const la_ = abs($la);
    const $lb = $l3 + $l4;
    const lb = qaq(l3,l4);
    const _lb = abs($lb);
    ...
    const $ln = $vₓᵧ*$li;
    const ln = qmq(vₓᵧ,li);
    const ln_ = vₓᵧ_*_li + 2*abs($ln);
    const $lo = $vₓₓ*$lj;
    const lo = qmq(vₓₓ,lj);
    const lo_ = vₓₓ_*_lj + 2*abs($lo);
    const $lp = $vᵧᵧ*$lk;
    const lp = qmq(vᵧᵧ,lk);
    const lp_ = vᵧᵧ_*_lk + 2*abs($lp);
    const $lq = c2*$vₓ;
    const lq = qmd(c2,vₓ);
    ...
    const _rg = abs($rg);
    const $rx = c1*$c0c0;
    const rx = qmd(c1,c0c0);
    const rx_ = abs($rx);
    const $rh = $rx*$vₓₓₓ;
    const rh = qmq(rx,vₓₓₓ);
    const rh_ = rx_*(_vₓₓₓ + vₓₓₓ_) + 2*abs($rh);
    const $ry = d1*$d0d0;
    const ry = qmd(d1,d0d0);
    const ry_ = abs($ry);
    const $ri = $ry*$vᵧᵧᵧ;
    const ri = qmq(ry,vᵧᵧᵧ);
    const ri_ = ry_*(_vᵧᵧᵧ + vᵧᵧᵧ_) + 2*abs($ri);
    const $rj = $vₓᵧ*$rc;
    const rj = qmq(vₓᵧ,rc);
    const rj_ = vₓᵧ_*_rc + 2*abs($rj);
    const $rk = $c0c1*$vₓₓ;
    const rk = qmq(c0c1,vₓₓ);
    const rk_ = _c0c1*vₓₓ_ + 2*abs($rk);
    const $rl = $d0d1*$vᵧᵧ;
    const rl = qmq(d0d1,vᵧᵧ);
    const rl_ = _d0d1*vᵧᵧ_ + 2*abs($rl);
    const $rm = $rk + $rl;
    const rm = qaq(rk,rl);
    ...
    const $rq = $rd*$rf;
    const rq = qmq(rd,rf);
    const rq_ = rd_*_rf + 2*abs($rq);
    const $rr = $re*$rg;
    const rr = qmq(re,rg);
    const rr_ = re_*_rg + 2*abs($rr);
    const $rs = $rq + $rr;
    const rs = qaq(rq,rr);
...
export { getCoeffsBez3Bez3DdWithRunningError47 }
