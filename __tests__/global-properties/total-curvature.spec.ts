...
function test(ps: number[][]) {
    {
        ...
        if (!closeTo([ulpsOrEps])(c - d, 0)) {
            // `ps: ${toString(ps)}; Total curvatures don't match: calc deg: ${radToDeg(c)}, gauss deg: ${radToDeg(d)} - calc rad: ${c}, gauss rad: ${d}`
            throw new Error(`Total curvatures don't match: calc deg: ${radToDeg(c)}, gauss deg: ${radToDeg(d)} - calc rad: ${c}, gauss rad: ${d}`);
        }
    }
...
function gaussCompare() {
...

    {
        ...
        //test(cusp);
        expect(totalCurvature(cusp.slice().reverse())).toBeNearly(2**4, 2.7030057599586934);
        expect(totalCurvature(cusp)).toBeNearly(2**4, -2.7030057599586934);
    }
...
function testBeziersInLoop(order: 1 | 2 | 3) {
    ...
    for (let s=seedSeed; s<seedSeed+seedCount; s++) {
        ...
        expect(totalWinding - Math.round(totalWinding)).toBeNearly([2**8],0);
    }
}
...
function testSomeTotalCurvature() {
    {
        ...
        const c = totalCurvature(ps, [0,1]);
        expect(c).toBeNearly(2**6, -0.9134823236058587);
    }

    {
        const ps = getRandomQuad(0);
        const c = totalCurvature(ps, [0,1]);
        expect(c).toBeNearly(2**6, 1.6188647958793885);
    }

    {
        const ps = getRandomCubic(2);
        const c = totalCurvature(ps, [0,1]);
        expect(c).toBeNearly(2**6, -5.178927914200722);
        const psr = ps.slice().reverse();
        const d = totalCurvature(psr, [0,1]);
        expect(d).toBeNearly(2**6, 5.178927914200722);
    }
...
describe('totalAbsoluteCurvature', function() {
    ...
    function() {
...

        {
            ...
            const d = totalAbsoluteCurvature(ps, [0,1]);
            expect(d).toBeNearly(2**4, Math.PI);
        }

        {
            ...
            expect(d).toBeNearly(2**6, 1.289670890832218);
            expect(e).toBeNearly(2**6, 1.289670890832218);
        }

        {
            ...
            const d = totalAbsoluteCurvature(ps, [0,1]);
            const e = totalAbsoluteCurvature(ps.slice().reverse(), [0,1]);
            expect(d).toBeNearly(2**6, 1.999553581786523);
            expect(e).toBeNearly(2**6, 1.999553581786523);
        }

        {
            ...
            const d = totalAbsoluteCurvature(ps, [0,1]);
            const e = totalAbsoluteCurvature(ps.slice().reverse(), [0,1]);
            expect(d).toBeNearly(2**6, 4.8964874516470225);
            expect(e).toBeNearly(2**6, 4.8964874516470225);
        }

        {
            ...
            const ps = getRandomQuad(0);
            const d = totalAbsoluteCurvature(ps, [0,1]);
            expect(d).toBeNearly(2**6, 1.6188647958793885);
        }
...

        {
            ...
            const p = [3,3];
            expect(() => totalAbsoluteCurvature([p,p,p,p,p], [0.5,0.5])).toThrow();
        }
...

        {
            ...
            const d = totalCurvature(ps, [0,1]);
            expect(d).toBeNearly(2**1,-𝜋/2);
        }

        {
            const p = [3,3];
            expect(() => totalCurvature([p,p,p,p,p], [0.5,1.5])).toThrow();
        }
    });
});
