...
describe('curviness', function() {
    ...
    function() {
        // cubics
        {
            ...
            expect(curviness(ps)).toEqual(0);
        }

        {
            const ps = [[0,0],[1.0001,1.001],[2.00001,2.00001],[3,3]];
            const curviness_ = curviness(ps);
            expect(curviness_).toBeNearly(2**8, 0.0013502388074583282);
        }

        {
            const ps = [[0,0],[2,1],[-2,25],[3,3]];
            const curviness_ = curviness(ps);
            expect(curviness_).toBeNearly(2**4, 4.355562125072504);
        }

        {
            const ps = [[0,0],[1,1],[0,0],[1,1]];
            const curviness_ = curviness(ps);
            expect(curviness_).toBeNearly(2**4, 6.283185307179586);
        }

        {
            const ps = [[0,0],[1,0],[0,0],[1,1]];
            const curviness_ = curviness(ps);
            expect(curviness_).toBeNearly(2**4, 5.497787143782138);
        }

        {
            const ps = [[0,0],[1,0],[0,0],[-1,0]];
            const curviness_ = curviness(ps);
            expect(curviness_).toBeNearly(2**4, 3.141592653589793);
        }
        ...
        // some edge cases
        {
            ...
            expect(curviness_).toEqual(0.3217505543966422);

            expect(curviness(ps1)).toEqual(curviness(ps2));
        }
