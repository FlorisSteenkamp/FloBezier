...
describe('isCollinear', function() {
    ...
    function() {
        {
            const ps = [[1,1]];
            expect(isCollinear(ps)).toEqual(true);
            ps[0][0] = ps[0][0] * (1 + eps);
            expect(isCollinear(ps)).toEqual(true);
        }
...

        {
            ...
            expect(isCollinear(ps)).toEqual(true);
            ps[0][0] = ps[0][0] * (1 + eps);
            expect(isCollinear(ps)).toEqual(false);
        }
        ...
        for (let i=0; i<10; i++) {
            const ps = getRandomLine(i);
            expect(isCollinear([ps[0]])).toEqual(true);
        }
...
describe('isVertical', function() {
    it('it should correctly check that some bezier control points are collinear and vertical', 
    function() {
        expect(isVertical([[1,0],[1,20]])).toEqual(true);
        expect(isVertical([[1,0],[1.0000000001,20]])).toEqual(false);

        expect(isVertical([[1,0],[1,1],[1,20]])).toEqual(true);
    });
});


describe('isHorizontal', function() {
    it('it should correctly check that some bezier control points are collinear and horizontal', 
    function() {
        expect(isHorizontal([[0,1],[20,1]])).toEqual(true);
        expect(isHorizontal([[0,1],[20,1.00001]])).toEqual(false);

        expect(isHorizontal([[0,1],[1,1],[20,1]])).toEqual(true);
    });
});