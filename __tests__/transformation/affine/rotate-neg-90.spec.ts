...
describe('rotateNeg90', function() {
    ...
    function() {
        for (let i=0; i<10; i++) {
            for (let order=1; order<=3; order++) {
                ...
                expect(rotateNeg90(ps)).toBeNearly([2**9], rotate(ps, -π/2));
            }
        }
    });
    ...
    function() {
        expect(rotateNeg90([[2,3], [-5,7]])).toEqual([[3,-2], [7,5]]);
    });

    it('it should be the exact identity after four applications',
    function() {
        const ps = [[2,3], [4,5], [6,7], [8,9]];
        expect(rotateNeg90(rotateNeg90(rotateNeg90(rotateNeg90(ps))))).toEqual(ps);
    });
});
