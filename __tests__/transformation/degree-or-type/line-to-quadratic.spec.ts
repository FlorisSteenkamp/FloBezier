    ...
    function() {
        {
            ...
            expect((y2 - y0)/2).toBeNearly(2**2, y2 - y1);
            expect((y2 - y0)/2).toBeNearly(2**2, y1 - y0);

            expect((x2 - x0)/2).toBeNearly(2**2, x2 - x1);
            expect((x2 - x0)/2).toBeNearly(2**2, x1 - x0);
        }
        ...
        // some edge cases
        {
            const p = [1,1];
            const ps = [p,p];
            expect(lineToQuadratic(ps)).toEqual([p,p,p]);
        }
    });
});
