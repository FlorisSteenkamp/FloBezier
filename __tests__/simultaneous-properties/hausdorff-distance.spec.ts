    ...
    function() {
        {
            ...
            expect(hd).toBeNearly(2**1,0.4867916480715152);
        }
        {
            ...
            expect(AB).toEqual(2);
            expect(BA).toBeNearly(2**1,(sqrt(3**2 + 2**2)));
            expect(hd).toEqual(BA);
            ...
            expect(AB_).toEqual(2);
            expect(BA_).toBeNearly(2**1,(sqrt(3**2 + 2**2)));
        }
        {
            // Test some line-quad
            ...
            expect(AB).toBeNearly(2**20,2);
            expect(BA).toBeNearly(2**1,(sqrt(3**2 + 2**2)));
            expect(hd).toEqual(BA);

            // estimate Hausdorff distance
            const hh = HH(A,B,10);
            expect(hd).toBeNearly(2**4,hh);
        }
        {
            ...
            expect(h).toBeNearly(2**32, 1.3749758408573243);
        ...
        }
        {
            ...
            expect(AB).toBeNearly(2**32,29.147780233490522);
            expect(BA).toBeNearly(2**32,27.36950949488669);

            // estimate Hausdorff distance
            const hAB = H(A,B,1);
            const hBA = H(B,A,1);
            expect(AB).toBeNearly(2**44,hAB);
            expect(BA).toBeNearly(2**44,hBA);
        }
        {
            // Test some quad-quad
            ...
            expect(AB).toBeNearly(2**32, 29.41592345293623);
            expect(BA).toBeNearly(2**32, 92.13090734616078);
            expect(h).toBeNearly(2**32, 92.13090734616078);
        }
        {
            // Test some quad-quad
            ...
            expect(AB).toBeNearly(2**32,34.29678192323552);
            expect(BA).toBeNearly(2**32,13.860492759353555);
        }
        {
...

        {
            ...
            expect(h).toBeNearly(2**32,0.0004163441673260007);
        }

        {
            ...
            expect(AB).toBeNearly(2**4, 1);
            expect(BA).toBeNearly(2**4, 2*sqrt(2));
        }
    });
});
