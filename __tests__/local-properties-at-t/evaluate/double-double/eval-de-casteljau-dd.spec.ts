...
describe('evalDeCasteljauDd', function() {
    it('it should evaluate some beziers correctly (in double-double precision) at some `t` values using De Casteljau\'s algorithm',
    function() {
        {
            const pss = [
                [[3,-1],[2,-1],[1,1],[0,0]],
            ...
            for (let ps of pss) {
                for (let t of ts) {
                    ...
                    expect(rX < eps**2 * 2**1).toBe(true);
                    expect(rY < eps**2 * 2**1).toBe(true);
                }
            }
        }
    });
