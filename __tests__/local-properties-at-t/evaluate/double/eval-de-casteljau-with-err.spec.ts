...
describe('evalDeCasteljauWithErr', function() {
    ...
    function() {
            ...
            for (let ps of pss) {
                for (let t of ts) {
                    ...
                    if (!(errActual <= errBound)) {
                        throw new Error(`error should be smaller than its bound, but error = ${errActual} whilst bound = ${errBound}`);
                    }

                    // error should be reasonably small (but depends on condition number)
                    const maxErrorBound = Number.EPSILON * 2**10;
                    if (!(errBound < maxErrorBound)) {
                        throw new Error(`error bound should be reasonably small (< ${maxErrorBound}) but is ${errBound}`);
                    }
                }
            }
        }
        {
            ...
            expect(r).toEqual({ p: [1/3,1/3], pE: [0,0] });
        }

        {
            const p = [1,1];
            const ps = [p,p,p,p,p];
            expect(() => evalDeCasteljauWithErr(ps,1)).toThrow();
        }
        
        {
            const p = [1,1];
            const ps = [p,p,p,p,p];
            expect(() => evalDeCasteljauError(ps,[0,0.5])).toThrow();
        }
    });
});
