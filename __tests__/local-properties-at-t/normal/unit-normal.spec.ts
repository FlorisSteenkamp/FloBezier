...
import { unitNormal } from '../../../src/local-properties-at-t/normal/double/unit-normal.js';
...
describe('unitNormal', function() {
    it('it should return a unit-length vector perpendicular to the tangent',
    function() {
        for (let i=0; i<10; i++) {
            for (let order=1; order<=3; order++) {
                ...
                for (const t of ts) {
                    const u = unitNormal(ps, t)!;
                    const v = tangent(ps, t);
                    const len = Math.sqrt(v[0]*v[0] + v[1]*v[1]);

                    // unit length
                    expect(Math.sqrt(u[0]*u[0] + u[1]*u[1])).toBeNearly(2**4, 1);
                    // the tangent rotated 90 degrees, then normalized
                    expect(u).toBeNearly(2**4, [-v[1]/len, v[0]/len]);
                }
            }
        }
    });
    ...
    function() {
        ...
        expect(unitNormal(ps, 0.5)).toBeUndefined();
    });
});
