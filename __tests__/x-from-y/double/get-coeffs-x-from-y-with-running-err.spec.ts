...
import { getCoeffsXFromY_WithRunningErr } from '../../../src/x-from-y/double/get-coeffs-x-from-y-with-running-err.js';
...
describe('getCoeffsXFromY_WithRunningErr', function() {
    it('it should get the polynomial coefficients (and a running error bound) whose roots are the `x` coordinates given a `y` coordinate of some bezier curves (in double precision) - and the actual errors should be within their bounds when compared to the exact (expansion) version',
    function() {
        {
            let k = 0;
            let maxRatio = 0;
            ...
            for (let i=1; i<=3; i++) {
                for (let j=0; j<20; j++) {
                    ...
                    const { coeffs, errorBound: _errorBound } =
                        getCoeffsXFromY_WithRunningErr(ps, y)!;
            ...
            console.log(`getCoeffsXFromY_WithRunningErr: average error/bound: ${sumRatio/count}, max error/bound: ${maxRatio}, average absolute error: ${sumAbsError/count}`);
        }
        ...
        // a point (order 0) has no x-from-y polynomial
        {
            const p = [1,1];
            const ps = [p];
            expect(getCoeffsXFromY_WithRunningErr(ps, 0)).toBeUndefined();
        }
    });
});
