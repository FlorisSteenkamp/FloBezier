...
import { ddGetCoeffsYFromX_WithRunningErr } from '../../../src/y-from-x/double-double/dd-get-coeffs-y-from-x-with-running-err.js';
...
describe('ddGetCoeffsYFromX_WithRunningErr', function() {
    it('it should get the polynomial coefficients (and a running error bound) whose roots are the `y` coordinates given an `x` coordinate of some bezier curves (in double-double precision) - and the actual errors should be within their bounds when compared to the exact (expansion) version',
    function() {
        {
            let k = 0;
            let maxRatio = 0;
            ...
            for (let i=1; i<=3; i++) {
                for (let j=0; j<20; j++) {
                    ...
                    const { coeffs: rDd, errorBound: _errorBound } =
                        ddGetCoeffsYFromX_WithRunningErr(ps, x)!;
            ...
            console.log(`ddGetCoeffsYFromX_WithRunningErr: average error/bound: ${sumRatio/count}, max error/bound: ${maxRatio}, average absolute error: ${sumAbsError/count}`);
        }
        ...
        // a point (order 0) has no y-from-x polynomial
        {
            const p = [1,1];
            const ps = [p];
            expect(ddGetCoeffsYFromX_WithRunningErr(ps, 0)).toBeUndefined();
        }
    });
});
