 ...
 * @returns {number[][]} The t-value pairs at intersection of the first 
 ...
 * this paper</a>
 */
function bezier3Intersection_sylvester(ps1, ps2) {
	let [x1, y1] = ps1[0];
	let [x2, y2] = ps2[0];
	...
	if (Math.abs(x1 - x2) > 1e-12) {
		...
		let rotatedPs1_ = Vector.rotatePs(sinθ, cosθ, ps1);
		let rotatedPs2_ = Vector.rotatePs(sinθ, cosθ, ps2);
	...
	// Cache
	let [c_3x, c_2x, c_1x, c_0x] = getX(rotatedPs1); 
	let [c_3y, c_2y, c_1y, c_0y] = getY(rotatedPs1);
	let [d_3x, d_2x, d_1x, d_0x] = getX(rotatedPs2); 
	let [d_3y, d_2y, d_1y, d_0y] = getY(rotatedPs2);
